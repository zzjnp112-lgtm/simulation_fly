...
def generate_launch_description():
    ...
    map_size_x = LaunchConfiguration('map_size_x_', default=30.0)
    map_size_y = LaunchConfiguration('map_size_y_', default=30.0)
    map_size_z = LaunchConfiguration('map_size_z_', default=4.0)
    ...
    cloud_topic = LaunchConfiguration('cloud_topic', default='/transformed_cloud') ######
    
    # 默认使用 SDF 中深度相机的内参 (width=640, height=480, hfov=1.4)
    cx = LaunchConfiguration('cx', default=320.0)
    cy = LaunchConfiguration('cy', default=240.0)
    fx = LaunchConfiguration('fx', default=379.73)
    fy = LaunchConfiguration('fy', default=284.80)
    
    max_vel = LaunchConfiguration('max_vel', default=0.5)
    max_acc = LaunchConfiguration('max_acc', default=0.5)
    planning_horizon = LaunchConfiguration('planning_horizon', default=19.0)
    
    point_num = LaunchConfiguration('point_num', default=2) ###航点数量，1代表一个航点
    # point0_x = LaunchConfiguration('point0_x', default=0.0) ##第一个点
    # point0_y = LaunchConfiguration('point0_y', default=0.0)
    # point0_z = LaunchConfiguration('point0_z', default=2.0)
    
    # point1_x = LaunchConfiguration('point1_x', default=5.0)
    # point1_y = LaunchConfiguration('point1_y', default=0.0)
    # point1_z = LaunchConfiguration('point1_z', default=2.0)
    ...
    ego_planner_node = Node(
        ...
        parameters=[
            ...
            {'fsm/waypoint_num': point_num},  # 目标点数量
            ...
            {'grid_map/resolution': 0.1}, #地图分辨率 ####0.1
            {'grid_map/map_size_x': map_size_x}, #地图在x方向的大小
            {'grid_map/map_size_y': map_size_y},#地图在y方向的大小
            {'grid_map/map_size_z': map_size_z},#地图在z方向的大小
            {'grid_map/local_update_range_x': 5.5},#本地地图更新范围，决定每次更新时考虑的空间大小。
            {'grid_map/local_update_range_y': 5.5},
            {'grid_map/local_update_range_z': 4.5},
            {'grid_map/obstacles_inflation': 0.35},#障碍物膨胀半径，增加障碍物的安全边界。
            {'grid_map/local_map_margin': 12}, #本地地图边缘的安全距离。
            {'grid_map/ground_height': 0.02},#地面高度阈值，低于此高度的点会被认为是地面。
            ...
            {'grid_map/depth_filter_tolerance': 0.15}, #度滤波容差。
            {'grid_map/depth_filter_maxdist': 5.0}, #深度滤波的最大距离
            {'grid_map/depth_filter_mindist': 0.2}, #深度滤波的最小距离。
            {'grid_map/depth_filter_margin': 2}, #深度滤波边缘宽度。
            {'grid_map/k_depth_scaling_factor': 1000.0}, #深度值缩放因子（通常用于将深度图像像素值转换为实际距离）。
            {'grid_map/skip_pixel': 2}, #深度图像跳过像素数。
            # local fusion
            {'grid_map/p_hit': 0.65},
            ...
            {'grid_map/p_max': 0.90},
            {'grid_map/p_occ': 0.80},
            {'grid_map/min_ray_length': 0.1},#射线最小长度，用于投射障碍物。
            {'grid_map/max_ray_length': 4.5},#射线最大长度，用于投射障碍物。
            
            {'grid_map/virtual_ceil_height': 8.0}, #虚拟天花板高度，超过此高度的点会被忽略。
            {'grid_map/visualization_truncate_height': 8.0},# 可视化截断高度，超过此高度的点不会显示在可视化中。
            {'grid_map/show_occ_time': False}, #是否显示占用时间。
            {'grid_map/pose_type': 1}, # 0: camera pose, 1: drone pose
            {'grid_map/frame_id': "world"}, # 坐标系ID，通常是世界坐标系。
            # {'grid_map/occupancy_inflate': 0.1}, #占用栅格膨胀半径。
            # {'grid_map/occupancy_inflate_time': 5.0}, #占用栅格膨胀时间。
            ...
            {'optimization/lambda_collision': 0.5}, ##0.5
